...
export async function POST(req: NextRequest) {
  try {
    ...
    // Derive from the same NEXT_PUBLIC_ var the client uses to load snap.js
    // (cart-context.tsx) so the token's environment can never drift from the
    // widget's environment — mismatched envs are what cause Midtrans's
    // "Transaksi tidak ditemukan" error.
    const IS_PRODUCTION =
